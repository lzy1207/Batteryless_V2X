...
% 准备一个 TGax 信道“模板”
% 默认信道配置，后面会 clone() 出多个独立信道对象，并针对不同仿真点改 DelayProfile / 天线数
...
% simParams. Each element contains the parameters for a simulation.(仿真参数结构体模板)
simParamsRef = struct('MCS',0,'SNR',0,'RandomSubstream',0,'Config',cfgHE, ...
    ...
    'Channel',tgaxChannel);
% 生成 simParams 结构体数组
% chans：信道模型（Model-B / Model-D）
% numTxRx：MIMO 配置（例如 [1 1], [4 2], [8 2]）
% mcs：MCS 列表
% snr：每个(信道模型, 天线配置, MCS) 对应的一组 SNR 点（查表得到）
% 对每个 SNR 点都生成一个 sp（一个仿真点）并 append 到 simParams
